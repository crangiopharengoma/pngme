...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Png> {
        let header: [u8; 8] = <[u8; 8]>::try_from(&value[0..8])?;
        if header != Self::STANDARD_HEADER {
            return Err(Box::new(InvalidBytesError(
                "invalid header bytes for PNG format".to_string(),
            )));
        }
        ...
        while start < value.len() {
            ...
            let length: u32 =
                u32::from_be_bytes(<[u8; 4]>::try_from(&value[start..length_offset])?);
            let end = start + Self::LENGTH_ADJUSTMENT + usize::try_from(length)?;
            data.push(Chunk::try_from(&value[start..end])?);
            start = end;
        }
...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        let index = self
            .data
            .iter()
            .position(|element| element.chunk_type().to_string() == chunk_type);
        if let Some(index) = index {
            Ok(self.data.remove(index))
        } else {
            Err(Box::new(ChunkNotFound(
                "could not find {chunk_type}".to_string(),
            )))
        }
    }
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        ...
        let interim = self
            .data
            .iter()
            .flat_map(move |element| element.as_bytes().to_vec());
        result.extend(interim);
        result
    }
}
